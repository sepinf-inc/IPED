 ...
 * juntos. Por exemplo, um módulo pode propor parsers, scripts, filtros
 * e outros que estão correlacionados. Esse módulo tem seus próprios dados,
 * configurações, arquivo de log e diretório de saída (necessário?).
 * No entanto, creio que ExaminationProcedures não deixa claro que esses
 * componentes podem ser de outros tipos. Não deveria ser ForensicModuleComponent?
 *
 * @param <A> The type of application the module is associated with.
 * @author werneck.bwph
 */
public interface ForensicModule<A extends App> extends Identity<UUID>, Module<A>, Serializable {
    ...
    <T extends ExaminationProcedure> T getExaminationProcedureInstance(
        Class<T> examinationProcedure) throws Exception;
}
