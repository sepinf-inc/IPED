...
public class GraphGenerator {
  ...
  private void runPostImportOps(File output) throws IOException {
    ...
    try {
      ...
      GraphConfiguration config = GraphTask.loadConfiguration(new File(Configuration.configPath, "conf"));
  ...
  private boolean importDB(File input, File output) {
    try {
      ...
      runner.run(output, GraphTask.DB_NAME, Configuration.outputOnSSD);
      return true;
    } catch (Exception e) {
      ...
      while (result.hasNext()) {
        ...
        if (currentContact != null && contact.getId() != currentContact.getId()) {
          ...
          for (Node inputNode : inputs.values()) {
            inputNode.createRelationshipTo(newGroup, DynRelationshipType.withName(config.getDefaultRelationship()));
          }

          for (Node evidence : evidences.values()) {
            newGroup.createRelationshipTo(evidence, DynRelationshipType.withName(config.getDefaultRelationship()));
          }

          newGroup.createRelationshipTo(currentContact, DynRelationshipType.withName(config.getDefaultRelationship()));
