...
public class PSTFile {
	...
	private static final String guidStrings[] = { "00020329-0000-0000-C000-000000000046", "00062008-0000-0000-C000-000000000046", "00062004-0000-0000-C000-000000000046",
			"00020386-0000-0000-C000-000000000046", "00062002-0000-0000-C000-000000000046", "6ED8DA90-450B-101B-98DA-00AA003F1305", "0006200A-0000-0000-C000-000000000046",
			"41F28F13-83F4-4114-A584-EEDB5A6B0BFF", "0006200E-0000-0000-C000-000000000046", "00062041-0000-0000-C000-000000000046", "00062003-0000-0000-C000-000000000046",
			"4442858E-A9E3-4E80-B900-317A210CC15B", "00020328-0000-0000-C000-000000000046", "71035549-0739-4DCB-9163-00F0580DBBDF", "00062040-0000-0000-C000-000000000046" };
	...
	private void processNameToIdMap(RandomAccessFile in) throws IOException, PSTException {
		...
		for (int i = 0; i < nGuids; ++i) {
			long mostSigBits = (PSTObject.convertLittleEndianBytesToLong(guids, offset, offset + 4) << 32) | (PSTObject.convertLittleEndianBytesToLong(guids, offset + 4, offset + 6) << 16)
		...
		for (int x = 0; x + 8 < nameToIdByte.length; x += 8) {
			...
			if ((wGuid & 0x0001) == 0) {
				...
				/*
				 * if (wPropIdx == 32784) { System.out.println("here!" +
				 * dwPropertyId); System.out.println(key);
	...
	/**
	 * get the root folder for the PST file. You should find all of your data
	 * under here...
	 * 
	 * @throws PSTException
	 * @throws IOException
	...
	/**
	 * Read a file offset from the file PST Files have this tendency to store
	 * file offsets (pointers) in 8 little endian bytes. Convert this to a long
	 * for seeking to.
	 * 
	 * @param in
	 *            handle for PST file
	...
	/**
	 * Build the children descriptor tree This goes through the entire
	 * descriptor B-Tree and adds every item to the childrenDescriptorTree. This
	 * is used as fallback when the nodes that list file contents are broken.
	 * 
	 * @param in
	 * @throws IOException
	 * @throws PSTException
