...
public class PSTObject {
	...
	static String createJavaString(byte[] data, int stringType, String codepage) {
		try {
			...
			if (codepage == null) {
				// patch Nassif: decodifica emails com múltiplas codificações
				return IOUtil.tryDecodeMixedCharset(data);
			} else {
			 ...
			 * codepage.toUpperCase().equals("UTF-7")) { // PST UTF-8 strings
			 * are not... really UTF-8 // it seems that they just don't use
			 * multibyte chars at all. // indeed, with some crylic chars in
			 * there, the difficult chars are just converted to %3F(?) // I
			 * suspect that outlook actually uses RTF to store these problematic
			 * strings. StringBuffer sbOut = new StringBuffer(); for (int x = 0;
			 * x < data.length; x++) { sbOut.append((char)(data[x] & 0xFF)); //
			 * just blindly accept the byte as a UTF char, seems right half the
			 * time } return new String(sbOut); } else { codepage =
			 * codepage.toUpperCase(); return new String(data, codepage); }
			 */
