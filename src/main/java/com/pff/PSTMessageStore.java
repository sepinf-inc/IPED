...
public class PSTMessageStore extends PSTObject {
	...
	public UUID getTagRecordKeyAsUUID() {
		...
		if (this.items.containsKey(guidEntryType)) {
			...
			long mostSigBits = (PSTObject.convertLittleEndianBytesToLong(bytes, offset, offset + 4) << 32) | (PSTObject.convertLittleEndianBytesToLong(bytes, offset + 4, offset + 6) << 16)
