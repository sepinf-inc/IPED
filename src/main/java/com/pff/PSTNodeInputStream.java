...
public class PSTNodeInputStream extends InputStream {
	...
	/**
	 * Read a block from the input stream. Recommended block size = 8176 (size
	 * used internally by PSTs)
	 * 
	 * @param output
	 * @return
	 * @throws IOException
	 ...
	 * int[this.skipPoints.size()]; for (int x = 0; x < this.skipPoints.size();
	 * x++) { out[x] = this.skipPoints.get(x).intValue(); } return out; }
	 */
