...
public class GalleryModel extends AbstractTableModel {
	...
	public Object getValueAt(final int row, final int col) {
		...
		if (!mediaType.startsWith("image") && !mediaType.endsWith("msmetafile") && !mediaType.endsWith("x-emf"))
			return new GalleryValue(doc.get(IndexItem.NAME), unsupportedIcon, docId);
		...
		executor.execute(new Runnable() {
			public void run() {
				...
				GalleryValue value = new GalleryValue(doc.get(IndexItem.NAME), null, docId);
				try {
					if (cache.containsKey(docId))
						return;
		...
		return new GalleryValue(doc.get(IndexItem.NAME), null, docId);
	}
