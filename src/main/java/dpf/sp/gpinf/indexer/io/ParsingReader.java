...
public class ParsingReader extends Reader {
  ...
  private class ParsingTask implements Runnable {
     ...
     * Potential exceptions (including the one caused if the read end is closed unexpectedly) are
  ...
  /**
   * Closes the read end of the pipe. If the parsing thread is still running, next write to the pipe
   * will fail and cause the thread to stop. Thus there is no need to explicitly terminate the
   * thread.
   *
