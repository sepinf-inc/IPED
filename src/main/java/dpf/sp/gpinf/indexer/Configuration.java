...
public class Configuration {
  ...
  public static String javaTmpDir = System.getProperty("java.io.tmpdir"); //$NON-NLS-1$
  ...
  public static void getConfiguration(String configPathStr) throws Exception {
    ...
    File newTmp = null, tmp = new File(System.getProperty("java.io.tmpdir")); //$NON-NLS-1$
    ...
    if (indexerTemp == null) {
      ...
      if (!indexerTemp.mkdirs()) {
        tmp = new File(System.getProperty("java.io.tmpdir")); //$NON-NLS-1$
        indexerTemp = new File(tmp, "indexador-temp" + new Date().getTime()); //$NON-NLS-1$
        indexerTemp.mkdirs();
      }
