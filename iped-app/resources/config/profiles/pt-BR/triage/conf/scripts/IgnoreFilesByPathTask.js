...
function process(e){
	...
	{
		//Nao ignora os arquivos em "arquivos de programas" das pastas da Caixa, Receita, ou outros de uma "lista branca"
		if(!(/((\\|\/)vol_vol.|sd..|md\d(\d\d)?(p\d)?)(\\|\/)(Arquivos de Programas( \(x86\))?|Program Files( \(x86\))?|Applications|opt)(\\|\/)(programa? rfb|caixa|cns|receita|rfb|irpf|dirf|dipj|dctf|dsimples|sped|perdcomp|sedif|simplesnacional|pgdcnpj|sefaz|sintegra|danfe|gdrais|sicalcp|cagednet)/i.test(path)))
		{
			e.setToIgnore(true);
		}
	}
