...
function process(e) {
	...
	*/
	...
	if (categorias.indexOf("PDF Documents") !== -1) {
		// DANFE / DACTE Detection
		try {
			var is = e.getSeekableInputStream();
			var buffer = new (Java.type("byte[]"))(4096);
			var len = is.read(buffer);
			if (len > 0) {
				var content = new java.lang.String(buffer, 0, len, java.nio.charset.StandardCharsets.ISO_8859_1);
				if (content.indexOf("DANFE") !== -1 || content.indexOf("Documento Auxiliar da Nota Fiscal") !== -1) {
					e.setMediaTypeStr("application/x-nfe+pdf");
				} else if (content.indexOf("DACTE") !== -1) {
