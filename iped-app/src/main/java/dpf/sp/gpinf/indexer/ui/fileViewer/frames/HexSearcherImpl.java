...
public class HexSearcherImpl implements HexSearcher{
	...
	public void doSearch(CodeArea codeArea, HighlightCodeAreaPainter painter, Hits hits,SeekableInputStream data,  Charset charset, Set<String> highlightTerms, long offset  ,boolean searchString, boolean ignoreCaseSearch, JLabel resultSearch, int max_hits) throws Exception{

		  GetResultsSearch getSearch = new GetResultsSearch(codeArea, painter, hits, data,  charset, highlightTerms, offset, searchString,  ignoreCaseSearch, resultSearch, max_hits);
		  getSearch.execute();
	...
	static class GetResultsSearch extends CancelableWorker<String, Integer>{
		...
		GetResultsSearch(CodeArea codeArea, HighlightCodeAreaPainter painter, Hits hits,SeekableInputStream data,  Charset charset, Set<String> highlightTerms, long offset  ,boolean searchString, boolean ignoreCaseSearch, JLabel resultSearch, int max_hits){
		...
		protected String doInBackground() throws Exception {
			...
			while (posicao < dataSize - bufferLength){
				...
				for(String texto : palavras){
					...
					if (n > 0){
						...
						hitsKMP = searchBytes(bufferTexto,buffer, bufferLength+texto.length()-1, (searchString && ignoreCaseSearch));	
				...
				if(progressMonitor != null){
					...
					progressMonitor.setNote("<html><body>"+ hitsEncontrados.size()+" "+ Messages.getString("HexSearcherImpl.hits")+"<br>"+timeLeftString+"</body></html>");
			...
			if (hits.totalHits > 0){
				...
				resultSearch.setText(Messages.getString("HexSearcherImpl.hit")+" "+(hits.currentHit+1)+" "+Messages.getString("HexSearcherImpl.of")+" "+hits.totalHits);			  
			}else{
	...
	*/
	 ...
	 * @param ignoreCaseSearch Case sensitive search 
	 * @return The list of starting indices of the matched word in the sentence. Empty list in case of no match.
	 */
	public static List<Integer> searchBytes(final byte[] word, final byte[] sentence, int length, boolean ignoreCaseSearch) {
	...
	public static List<Integer> createPartialMatchTable(final byte[] word) {
		if (isBlank(word)) return Collections.EMPTY_LIST;
		...
		for (int idx = 1; idx < word.length; idx++) {
			...
			if (prevVal == 0) {
				if (word[idx] == firstChar) partialTable.add(1);
				else partialTable.add(0);
			} else if (word[idx] == word[prevVal]) partialTable.add(prevVal + 1);
			else partialTable.add(0);
		}
