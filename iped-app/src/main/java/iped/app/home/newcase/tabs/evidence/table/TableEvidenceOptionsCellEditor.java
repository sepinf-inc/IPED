...
/**
 * The Evidence table cell editor
 * this class is used to create a custom cell to the Evidence table
 * here we get the evidence info edit button (... button) and the remove row button (x button)
 */
public class TableEvidenceOptionsCellEditor extends AbstractCellEditor implements TableCellEditor {
    ...
    /**
     * TableEvidenceOptionsCellEditor constructor
     * this class gonna be instantiated only if user edit the options column cell
     * @param evidencesList - the list of evidences used in the evidences table
     * @param infoDialog - The evidence editor dialog reference
     */
    ...
    /**
     * @param table - Evidence table instance
     * @param value - Evidence to be edited object reference
     * @param isSelected - is column selected?
    ...
    /**
     * Create a panel with two buttons, one for evidence info dialog and other to remove selected row
     * @param rowIndex - The evidence table current row
     * @param table - the evidence table
     * @return - A JPanel containing two buttons to edit or remove
     */
    private JPanel createOptionPanel(int rowIndex, JTable table){
        ...
        removeButton.addActionListener( e -> {evidencesList.remove(rowIndex); ((EvidencesTableModel) table.getModel()).fireTableDataChanged(); fireEditingStopped(); });
