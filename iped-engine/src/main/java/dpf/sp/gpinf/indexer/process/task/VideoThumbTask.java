...
public class VideoThumbTask extends ThumbTask {
    ...
    private void saveMetadata(VideoProcessResult r, Metadata metadata) {
        ...
        if (duration != -1)
            metadata.set("duration", Double.toString(duration / 1000)); // $NON-NLS-1$
        Dimension d = r.getDimension();
        if (d != null) {
            metadata.set("width", Integer.toString(d.width)); // $NON-NLS-1$
            metadata.set("height", Integer.toString(d.height)); // $NON-NLS-1$
        }
