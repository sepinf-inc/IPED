 ...
 *
 */
public class Zip4jFastHeaderReader {
     ...
     * @param zip4jRaf
     */
    public Zip4jFastHeaderReader(RandomAccessFile zip4jRaf) {
     ...
     * input charset name. If this parameter is null, default system charset is
     * used. <br>
     * <br>
     * <b>Note:</b> This method does not read local file header information
     * 
    ...
    public ZipModel readAllHeaders(String fileNameCharset) throws ZipException {
        ...
        // zipModel.setLocalFileHeaderList(readLocalFileHeaders()); //Donot read local
        // headers now.
        return zipModel;
    }
    ...
    private EndCentralDirRecord readEndOfCentralDirectoryRecord() throws ZipException {
        ...
        try {
            ...
            throw new ZipException("Probably not a zip file or a corrupted zip file", e,
                    ZipExceptionConstants.notZipFile);
        }
    }
    ...
    private CentralDirectory readCentralDirectory() throws ZipException {
        ...
        try {
            ...
            byte[] fullBuf = new byte[46];

            for (int i = 0; i < centralDirEntryCount; i++) {
                FileHeader fileHeader = new FileHeader();

                readIntoBuff(zip4jRaf, fullBuf);

                // FileHeader Signature
                // readIntoBuff(zip4jRaf, intBuff);
                System.arraycopy(fullBuf, 0, intBuff, 0, 4);
                ...
                // version made by
                // readIntoBuff(zip4jRaf, shortBuff);
                System.arraycopy(fullBuf, 4, shortBuff, 0, 2);
                fileHeader.setVersionMadeBy(Raw.readShortLittleEndian(shortBuff, 0));

                // version needed to extract
                // readIntoBuff(zip4jRaf, shortBuff);
                System.arraycopy(fullBuf, 6, shortBuff, 0, 2);
                fileHeader.setVersionNeededToExtract(Raw.readShortLittleEndian(shortBuff, 0));

                // general purpose bit flag
                // readIntoBuff(zip4jRaf, shortBuff);
                System.arraycopy(fullBuf, 8, shortBuff, 0, 2);
                fileHeader.setFileNameUTF8Encoded(
                        (Raw.readShortLittleEndian(shortBuff, 0) & InternalZipConstants.UFT8_NAMES_FLAG) != 0);
    ...
    private void readAndSaveZip64ExtendedInfo(LocalFileHeader localFileHeader) throws ZipException {
        ...
        Zip64ExtendedInfo zip64ExtendedInfo = readZip64ExtendedInfo(localFileHeader.getExtraDataRecords(),
                localFileHeader.getUncompressedSize(), localFileHeader.getCompressedSize(), -1, -1);
     ...
     * @throws ZipException
     */
    private Zip64ExtendedInfo readZip64ExtendedInfo(ArrayList extraDataRecords, long unCompressedSize,
    ...
    /**
     * Sets the current random access file pointer at the start of signature of the
    ...
    private byte[] readIntoBuff(RandomAccessFile zip4jRaf, byte[] buf) throws ZipException {
        try {
            int read = 0, off = 0;
            while (off < buf.length && (read = zip4jRaf.read(buf, off, buf.length - off)) != -1) {
                off += read;
            }
            if (off < buf.length) {
