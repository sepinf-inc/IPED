...
public class PathToGuidConverter {
    ...
    private static final Map<String, List<String>> PATH_KEY_TO_GUIDS_MAP = new TreeMap<>((e1, e2) -> {
    ...
    static {
        ...
        PATH_KEY_TO_GUIDS_MAP.put("%PROGRAMFILES%", unmodifiableList(programFilesGuids));
        ...
        PATH_KEY_TO_GUIDS_MAP.put("%PROGRAMFILES(X86)%", unmodifiableList(programFilesX86Guids));
        ...
        PATH_KEY_TO_GUIDS_MAP.put("%COMMONPROGRAMFILES%", unmodifiableList(commonProgramFilesGuids));
        ...
        PATH_KEY_TO_GUIDS_MAP.put("%COMMONPROGRAMFILES(X86)%", unmodifiableList(commonProgramFilesX86Guids));
        ...
        PATH_KEY_TO_GUIDS_MAP.put("%WINDIR%\\SYSTEM32", unmodifiableList(system32Guids));
        PATH_KEY_TO_GUIDS_MAP.put("%WINDIR%\\SYSWOW64", singletonList("{D65231B0-B2F1-4857-A4CE-A8E7C6EA7D27}")); // FOLDERID_SystemX86

        // ProgramData
        PATH_KEY_TO_GUIDS_MAP.put("%PROGRAMDATA%", singletonList("{62AB5D82-FDC1-4DC3-A9DD-070D1D495D97}")); // FOLDERID_ProgramData
        PATH_KEY_TO_GUIDS_MAP.put("%ALLUSERSPROFILE%", singletonList("{62AB5D82-FDC1-4DC3-A9DD-070D1D495D97}")); // FOLDERID_ProgramData

        // Common single environment variables
        PATH_KEY_TO_GUIDS_MAP.put("%WINDIR%", singletonList("{F38BF404-1D43-42F2-9305-67DE0B28FC23}")); // FOLDERID_Windows
        PATH_KEY_TO_GUIDS_MAP.put("%USERPROFILE%", singletonList("{5E6C858F-0E22-4760-9AFE-EA3317B67173}")); // FOLDERID_Profile
        PATH_KEY_TO_GUIDS_MAP.put("%APPDATA%", singletonList("{3EB685DB-65F9-4CF6-A03A-E3EF65729F3D}")); // FOLDERID_RoamingAppData
        PATH_KEY_TO_GUIDS_MAP.put("%LOCALAPPDATA%", singletonList("{F1B32785-6FBA-4FCF-9D55-7B8E7F157091}")); // FOLDERID_LocalAppData
        PATH_KEY_TO_GUIDS_MAP.put("%PUBLIC%", singletonList("{DFDF76A2-C82A-4D63-906A-5644AC457385}")); // FOLDERID_Public

        // Paths that can contain executables (including composite paths)

        // Downloads
        PATH_KEY_TO_GUIDS_MAP.put("%PUBLIC%\\DOWNLOADS", singletonList("{3D644C9B-1FB8-4f30-9B45-F670235F79C0}")); // FOLDERID_PublicDownloads
        PATH_KEY_TO_GUIDS_MAP.put("%USERPROFILE%\\DOWNLOADS", singletonList("{374DE290-123F-4565-9164-39C4925E467B}")); // FOLDERID_Downloads

        // Desktop
        PATH_KEY_TO_GUIDS_MAP.put("%USERPROFILE%\\DESKTOP", singletonList("{B4BFCC3A-DB2C-424C-B029-7FE99A87C641}")); // FOLDERID_Desktop
        PATH_KEY_TO_GUIDS_MAP.put("%PUBLIC%\\DESKTOP", singletonList("{C4AA340D-F20F-4863-AFEF-F87EF2E6BA25}")); // FOLDERID_PublicDesktop

        // UserProgramFiles
        PATH_KEY_TO_GUIDS_MAP.put("%LOCALAPPDATA%\\PROGRAMS", singletonList("{5CD7AEE2-2219-4A67-B85D-6C9CE15660CB}")); // FOLDERID_UserProgramFiles
        PATH_KEY_TO_GUIDS_MAP.put("%LOCALAPPDATA%\\PROGRAMS\\COMMON", singletonList("{BCBD3057-CA5C-4622-B42D-BC56DB0AE516}")); // FOLDERID_UserProgramFilesCommon
    }
    ...
    public List<String> pathsWithKnownFolderIds(String genericPath) {
        ...
        for (Map.Entry<String, List<String>> entry : PATH_KEY_TO_GUIDS_MAP.entrySet()) {
