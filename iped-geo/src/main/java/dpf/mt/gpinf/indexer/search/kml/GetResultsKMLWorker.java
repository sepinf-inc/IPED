...
public class GetResultsKMLWorker extends iped3.desktop.CancelableWorker<String, Integer>{
	...
	protected String doInBackground() throws Exception {
		  ...
		  query +=  "("+GeofileParser.LATITUDE.getName().replace(":", "\\:")+":[-90 TO 90] AND "+GeofileParser.LONGITUDE.getName().replace(":", "\\:")+":[-180 TO 180])";
	...
	private void generateLocationKML(StringBuilder tourPlayList, StringBuilder kml, String coluna,
        ...
        if(alt==null) {
            ...
            kml.append("<Point><altitudemode>absolute</altitudemode><extrude>1</extrude><coordinates>"+longit+","+lat+","+alt+"</coordinates></Point>"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
        }
