...
public class PSTObject {
    ...
    protected PSTObject(PSTFile theFile, DescriptorIndexNode descriptorIndexNode) throws PSTException, IOException {
        ...
        PSTTableBC table = new PSTTableBC(new PSTNodeInputStream(pstFile,
                pstFile.getOffsetIndexNode(descriptorIndexNode.dataOffsetIndexIdentifier)));
     ...
     * @param table
     */
    protected PSTObject(PSTFile theFile, DescriptorIndexNode folderIndexNode, PSTTableBC table,
    ...
    /**
     * get the descriptor identifier for this item can be used for loading objects
    ...
    static String createJavaString(byte[] data, int stringType, String codepage) {
        try {
             ...
             * codepage.toUpperCase().equals("UTF-7")) { // PST UTF-8 strings are not...
             * really UTF-8 // it seems that they just don't use multibyte chars at all. //
             * indeed, with some crylic chars in there, the difficult chars are just
             * converted to %3F(?) // I suspect that outlook actually uses RTF to store
             * these problematic strings. StringBuffer sbOut = new StringBuffer(); for (int
             * x = 0; x < data.length; x++) { sbOut.append((char)(data[x] & 0xFF)); // just
             * blindly accept the byte as a UTF char, seems right half the time } return new
             * String(sbOut); } else { codepage = codepage.toUpperCase(); return new
             * String(data, codepage); }
             */
    ...
    /**
     * These are the common properties, some don't really appear to be common across
     * folders and emails, but hey
     */
     ...
     * int[] output = new int[numberOfEntries]; int tableOffset = 8; int blockOffset
     * = 0; for (int y = 0; y < numberOfEntries; y++) { // get the offset identifier
     * long tableOffsetIdentifierIndex =
     * PSTObject.convertLittleEndianBytesToLong(data, tableOffset, tableOffset+8);
    ...
    static PSTObject detectAndLoadPSTObject(PSTFile theFile, DescriptorIndexNode folderIndexNode)
        ...
        if (nidType == 0x02 || nidType == 0x03 || nidType == 0x04) {
            ...
            PSTTableBC table = new PSTTableBC(new PSTNodeInputStream(theFile,
    ...
    static PSTMessage createAppropriatePSTMessageObject(PSTFile theFile, DescriptorIndexNode folderIndexNode,
            PSTTableBC table, HashMap<Integer, PSTDescriptorItem> localDescriptorItems) {
    ...
    static String guessPSTObjectType(PSTFile theFile, DescriptorIndexNode folderIndexNode)
        ...
        PSTTableBC table = new PSTTableBC(
    ...
    /**
     * the code below was taken from a random apache project http://www.koders.com
     * /java/fidA9D4930E7443F69F32571905DD4CA01E4D46908C.aspx my bit-shifting isn't
     * that 1337
     */

    /**
     * <p>
     * The difference between the Windows epoch (1601-01-01 00:00:00) and the Unix
     * epoch (1970-01-01 00:00:00) in milliseconds: 11644473600000L. (Use your
     * favorite spreadsheet program to verify the correctness of this value. By the
     * way, did you notice that you can tell from the epochs which operating system
     * is the modern one? :-))
     * </p>
     */
     ...
     * which have passed since January 1, 1601. This 64-bit value is split into the
     * two double words stored in the structure.
     * </p>
     * 
     * @param high
