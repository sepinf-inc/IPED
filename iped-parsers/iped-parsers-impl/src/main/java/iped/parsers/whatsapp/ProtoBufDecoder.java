...
public class ProtoBufDecoder {
    ...
    private int readInt32BE() {
        int ret = ((bytes[pos] & 0xFF) << 24) | ((bytes[pos + 1] & 0xFF) << 16) | ((bytes[pos + 2] & 0xFF) << 8) | ((bytes[pos + 3] & 0xFF) << 0);
        pos += 4;
        return ret;
    }

    private int readInt32LE() {
        int ret = ((bytes[pos] & 0xFF) << 0) | ((bytes[pos + 1] & 0xFF) << 8) | ((bytes[pos + 2] & 0xFF) << 16) | ((bytes[pos + 3] & 0xFF) << 24);
        pos += 4;
        return ret;
    }
