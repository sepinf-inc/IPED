...
public class ShareazaDownloadParser extends AbstractParser {
	...
	public void parse(InputStream stream, ContentHandler handler, Metadata metadata, ParseContext context)
		...
		try {
			...
			processSDFile(stream, xhtml, searcher);
	...
	public void processSDFile(InputStream inputStreamFile, XHTMLContentHandler xhtml, IItemSearcher searcher)
		...
		try {
			...
			if (sha1Valid != 0) {
				...
				searchByHash(xhtml, searcher, HASH_SHA1, hash);
			}
			...
			if (version >= 22) {
				...
				if (md5Valid != 0) {
					...
					searchByHash(xhtml, searcher, HASH_MD5, hash);
				}
			...
			if (version >= 13) {
				...
				if (edonkeyValid != 0) {
					...
					searchByHash(xhtml, searcher, HASH_EDONKEY, hash);
				}
			...
			if (version >= 22) {
				...
				if (torrentValid > 0) {
					...
					int lenght = read4Bytes(buffer);
					addLine(xhtml, "    Lenght:                " + lenght);

					if (lenght > 0) {
						String nSource = readHashString(buffer, lenght);
					...
					if (version >= 23) {
						...
						addLine(xhtml, "BTH HASH:                       " + readHashString(buffer, 20));
						addLine(xhtml, "SEENDING:                       " + read2Bytes(buffer));
					}
				}
			...
			if (ed2kListSize > 0) {
				...
				addLine(xhtml, "     EDONKEY [HASHSET BLOCK LENGHT]:     " + hashsetBlock);
			...
			addLine(xhtml, "Error during file parser, possible incomplete or corrupted file.");
			ex.printStackTrace();
			return;

		} finally {
			addLine(xhtml, "</pre></body></html>");
		}
	...
	public void padding(ByteBuffer buffer, int i) {
	...
	public String readString(ByteBuffer buffer) {
		...
		buffer.get(read);
		try {
			int lenght = readMiniHeader(read);
			if (lenght > 0) {
				byte[] stringBytes = new byte[lenght * 2];
	...
	public long read8Bytes(ByteBuffer buffer) {
	...
	public int readMiniHeader(byte[] ldata) {
		...
		if (h1 != 0xFF || h2 != 0xFE || h3 != 0xFF) {
			throw new RuntimeException("ERROR: not found 'header' FF-FE-FF");
		}
		return len;
	}
	...
	public void readXMLNode(ByteBuffer buffer, XHTMLContentHandler xhtml) throws IOException, SAXException {
	...
	public void searchByHash(XHTMLContentHandler xhtml, IItemSearcher searcher, String hashType, String hash)
			throws SAXException {

		HashSet<String> hashSets = new HashSet<String>();
		hashSets.addAll(ChildPornHashLookup.lookupHash(hashType, hash.toUpperCase()));

		IItemReader item = searchItemInCase(searcher, hashType, hash.toUpperCase());
		if (item != null)
		...
		if (hash != null && !hash.isEmpty())
			attributes.addAttribute("", "name", "name", "CDATA", hash.toUpperCase());
