...
public class LNKShortcut {
    ...
    public static enum FileAttributeFlags {
        READONLY(0x00000001), HIDDEN(0x00000002), SYSTEM(0x00000004), RESERVED_1(0x00000008), // Reserved, not used by
                                                                                              // the LNK format Is a
                                                                                              // volume label
        DIRECTORY(0x00000010), ARCHIVE(0x00000020), DEVICE(0x00000040), NORMAL(0x00000080), TEMPORARY(
                0x00000100), SPARSE_FILE(0x00000200), REPARSE_POINT(0x00000400), COMPRESSED(0x00000800), OFFLINE(
                        0x00001000), NOT_CONTENT_INDEXED(0x00002000), ENCRYPTED(0x00004000), RESERVED_2(0x00000008), // Unknown
                                                                                                                     // (seen
                                                                                                                     // on
                                                                                                                     // Windows
                                                                                                                     // 95
    ...
    public static enum DataFlags {
        ...
        IsUnicode(0x00000080), // The data strings in the LNK file are stored in Unicode (UTF-16 little-endian)
                               // instead of ASCII
        ...
        Reserved_1(0x00000800), HasDarwinID(0x00001000), // The LNK file contains a Darwin (Mac OS-X) properties data
                                                         // block
        ...
        NoPidlAlias(0x00008000), // The file system location is represented in the shell namespace when the path
                                 // to an item is parsed into the link target identifiers Contains a known folder
                                 // location data block?
        Reserved_2(0x00010000), RunWithShimLayer(0x00020000), // The target application is run with the shim layer. The
                                                              // LNK file contains shim layer properties data block.
    ...
    public static String[] getDataFlagArray(int dFlag) {
        return Arrays.stream(DataFlags.values())
            .filter(enumItem -> (dFlag & enumItem.flag) == enumItem.flag)
            .map(enumItem -> enumItem.name())
            .toArray(String[]::new);
    }
