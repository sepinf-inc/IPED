...
public class DiscordHTMLReport {
    ...
    public void printHTML(List<DiscordRoot> drl, XHTMLContentHandler xHandler, IItemSearcher searcher) throws IOException {
        ...
        try {
            ...
            xHandler.startElement("script src=\"https://unpkg.com/@lottiefiles/lottie-player@latest/dist/lottie-player.js\"");
            ...
            for (DiscordRoot dr : drl) {
                ...
                if (!dr.getAuthor().getId().equals(userID)) {
                    ...
                    if (dr.getAuthor().getAvatarBytes() == null) {
                        ...
                        if (avatar == null || avatar.trim().equals("")) {
                            ...
                            xHandler.startElement("img src='data:image/png;base64, " + defaultAvatar + "' alt='' width='50' height='50'");
                            xHandler.endElement("img");
                        } else {
                            xHandler.startElement("img src='https://cdn.discordapp.com/avatars/" + format(dr.getAuthor().getId()) + "/" + format(avatar) + ".png' alt='' width='50' height='50'");
                            xHandler.endElement("img");
                        }
                    } else {
                        xHandler.startElement("img src='data:image/jpeg;base64, " + Base64.getEncoder().encodeToString(dr.getAuthor().getAvatarBytes()) + "' alt='' width='50' height='50'");
                        xHandler.endElement("img");
                    }

                    xHandler.endElement("TD");
                    xHandler.startElement("TD");
                    xHandler.startElement("span title='Channel ID=" + format(dr.getChannel_id()) + ", UserID=" + format(dr.getAuthor().getId()) + "'");
                ...
                xHandler.characters(dr.getEditedTimestamp() == null ? "" : "(" + Messages.getString("DiscordParser.EditTime") + dateFormat.format(dr.getEditedTimestamp()) + ")");
                ...
                if (stickers != null) {
                    for (DiscordSticker sticker : stickers) {
                        ...
                        xHandler.startElement(
                                "lottie-player src=\"data:application/json;base64, " + Base64.getEncoder().encodeToString(buff) + "\" background=\"transparent\"  speed=\"1\"  style=\"width: 300px; height: 300px;\" loop controls autoplay");
                ...
                if (dr.getCall() != null) {
                    ...
                    xHandler.endElement("TD");
                    xHandler.startElement("TD");
                    xHandler.characters(dr.getCall().getEndedTimestamp() == null ? "" : dateFormat.format(dr.getCall().getEndedTimestamp()));
                    xHandler.endElement("TD");
                    xHandler.endElement("TR");
                    xHandler.startElement("TR");
                    xHandler.startElement("TD");
                ...
                if (dr.getReactions() != null) {
                    ...
                    xHandler.startElement("TR");
                    xHandler.startElement("TD");
                    xHandler.characters(Messages.getString("DiscordParser.Reactions") + String.join(", ", reactions));
                    xHandler.endElement("TD");
                    xHandler.endElement("TR");
                    xHandler.endElement("TABLE");
                }
                ...
                if (dr.getAttachments() != null && dr.getAttachments().size() > 0) {
                    ...
                    for (DiscordAttachment att : dr.getAttachments()) {
                        ...
                        if (att.getMediaHash() != null) {
                            ...
                            xHandler.startElement("input class=\"check\" type=\"checkbox\" onclick=\"app.check('hash:" + hash + "', this.checked)\" name=\"" + hash + "\"");
                            xHandler.endElement("input");
                            xHandler.startElement("a onclick=\"app.open('hash:" + att.getMediaHash() + "')\" href='" + Util.getExportPath(item) + "'");
                            if (item.getThumb() != null) {
                                xHandler.startElement("img src=\"data:image/jpeg;base64," + Base64.getEncoder().encodeToString(item.getThumb()) + "\" title=\"" + format(att.getFilename()) + "\"");
                                xHandler.endElement("img");
                            ...
                            if (item.getMediaType().toString().startsWith("video/")) {
                                ...
                                if (thumb != null) {
                                    xHandler.startElement("img class=\"thumb iped-video\" src=\"" + "data:image/jpg;base64," + iped.parsers.whatsapp.Util.encodeBase64(thumb) + "\"" + " data-src1=\"" + format(exportPath) + "\""
                                            + " data-src2=\"" + format(source) + "\"" + " title=\"" + att.getFilename() //$NON-NLS-2$ //$NON-NLS-3$
                                            + "\"");
                                    xHandler.endElement("img");

                                } else {
                                    xHandler.startElement("div class=\"videoImg iped-video\" title=\"Video\"" + " data-src1=\"" + format(exportPath) + "\"" + " data-src2=\"" + format(source) + "\"");
                                    xHandler.startElement("div");
                                }
                            }
                        ...
                        if (!att.getChildPornSets().isEmpty()) {
                            ...
                            xHandler.characters(Messages.getString("WhatsAppReport.FoundInPedoHashDB") + " " + format(att.getChildPornSets().toString()));
