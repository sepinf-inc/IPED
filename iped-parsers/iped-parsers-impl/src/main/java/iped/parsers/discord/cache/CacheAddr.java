...
public class CacheAddr {
	...
	public String toString() {
		return "CacheAddr [address=" + address + ", initialized=" + initialized + ", fileType=" + fileType
				+ ", numBlocks=" + numBlocks + ", fileSelector=" + fileSelector + ", startBlock=" + startBlock
				+ ", fileName=" + fileName + ", fileNameStr=" + fileNameStr + "]";
	}
	...
	public InputStream getInputStream(List<IItemReader> dataFiles, List<IItemReader> externalFiles,
			Integer dataStreamSize) throws IOException {
		if (!initialized) {
		...
		case 4:
			for (IItemReader dataFile : dataFiles)
				if (dataFile.getName().equals(("data_" + fileSelector))) {
					...
					// zero bytes
					byte[] blocks = (dataStreamSize == null) ? new byte[(numBlocks + 1) * getBlockSize()]
							: new byte[dataStreamSize];
					sis.read(blocks);
