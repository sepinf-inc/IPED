...
public class RecipientTable extends AbstractTable {
    ...
    private RecipientEntry getRecipient(EsedbLibrary esedbLibrary, int i, PointerByReference errorPointer, PointerByReference tablePointerReference) {
        ...
        messageId = EsedbManager.getInt32Value(esedbLibrary, 3, recordPointerReference, filePath, errorPointer);
        displayName = EsedbManager.getUnicodeValue(esedbLibrary, 12, recordPointerReference, filePath, errorPointer);
        recipientType = EsedbManager.getInt32Value(esedbLibrary, 4, recordPointerReference, filePath, errorPointer);
        ...
        return new RecipientEntry(messageId, displayName, RecipientType.values()[recipientType]);
    }
}
