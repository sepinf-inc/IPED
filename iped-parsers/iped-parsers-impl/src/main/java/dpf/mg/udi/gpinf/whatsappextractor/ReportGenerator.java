...
public class ReportGenerator {
	...
	public byte[] generateNextChatHtml(Chat c, WAContactsDirectory contactsDirectory)
		...
		if (currentMsg > 0)
			out.println("<div class=\"linha\"><div class=\"date\">" + Messages.getString("WhatsAppReport.ChatContinuation") + "</div></div>"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
		...
		while (currentMsg < c.getMessages().size()) {
			...
			if (currentMsg++ != c.getMessages().size() - 1 && bout.size() >= MAX_CHAT_SIZE) {
				out.println("<div class=\"linha\"><div class=\"date\">" + Messages.getString("WhatsAppReport.ChatContinues") + "</div></div>"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
				break;
			}
		}
	...
	private void printMessage(PrintWriter out, Message message, boolean group, WAContactsDirectory contactsDirectory) {
		...
		case VIDEO_CALL:
			...
			out.println(Messages.getString("WhatsAppReport.Duration") + ": " + formatMMSS(message.getMediaDuration())); //$NON-NLS-1$ //$NON-NLS-2$
			break;
		case VOICE_CALL:
			if (message.isFromMe()) {
				out.println("<div class=\"specialmessage to\">"); //$NON-NLS-1$
		...
		case GROUP_CREATED:
			out.println("<div class=\"systemmessage\">"); //$NON-NLS-1$
			out.println(Messages.getString("WhatsAppReport.GroupCreated") + " " + message.getRemoteResource() + "</br>"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
		...
		case USERS_JOINED_GROUP:
			out.println("<div class=\"systemmessage\">"); //$NON-NLS-1$
			out.println(Messages.getString("WhatsAppReport.UserJoinedGroup") + message.getRemoteResource() + "</br>"); //$NON-NLS-1$ //$NON-NLS-2$
		...
		case USER_REMOVED_FROM_GROUP:
			out.println("<div class=\"systemmessage\">"); //$NON-NLS-1$
			out.println(Messages.getString("WhatsAppReport.UserRemovedGroup") + message.getRemoteResource() + "</br>"); //$NON-NLS-1$ //$NON-NLS-2$
			break;
		...
		default:
			...
			case DELETED_FROM_SENDER:
				out.println("<i>" + Messages.getString("WhatsAppReport.MessageDeletedFromSender") + "</i><br/>"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
				break;
			...
			case GIF_MESSAGE:
				if (message.getMediaHash() != null) {
					...
					result = dpf.sp.gpinf.indexer.parsers.util.Util.getItems("sha-256:" + message.getMediaHash(), searcher); //$NON-NLS-1$
					...
					if(searcher != null) mediaName = searcher.escapeQuery(mediaName);
					String query = BasicProps.NAME + ":\"" + mediaName + "\" AND " + BasicProps.LENGTH + ":" + message.getMediaSize(); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
					onclick="\"app.open(" + SimpleHTMLEncoder.htmlEncode("\"" + query.replace("\"", "\\\"") + "\"") + ")\" "; //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$ //$NON-NLS-5$ //$NON-NLS-6$
					if(searcher != null)
		...
		if (message.isFromMe() && message.getMessageStatus() != null) {
			out.println("<div style=\"display: inline-block;\"><svg id=\"Layer_1\" xmlns=\"http://www.w3.org/2000/svg\" viewBox=\"0 0 16 15\" width=\"16\" height=\"15\">"); //$NON-NLS-1$
			...
			case MESSAGE_UNSENT:
				...
				break;
			case MESSAGE_SENT:
				out.println("<path fill=\"#92A58C\" d=\"M10.91 3.316l-.478-.372a.365.365 0 0 0-.51.063L4.566 9.879a.32.32 0 0 1-.484.033L1.891 7.769a.366.366 0 0 0-.515.006l-.423.433a.364.364 0 0 0 .006.514l3.258 3.185c.143.14.361.125.484-.033l6.272-8.048a.365.365 0 0 0-.063-.51z\"></path>"); //$NON-NLS-1$
				break;
