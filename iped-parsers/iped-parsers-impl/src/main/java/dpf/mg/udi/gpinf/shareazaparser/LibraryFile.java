...
class LibraryFile extends ShareazaEntity {
	...
	public void printTableRow(XHTMLContentHandler html, String path, ItemSearcher searcher) throws SAXException {
		...
		if(LedHashes.hashMap != null){
			if((md5 != null && md5.length() == 32 && Arrays.binarySearch(LedHashes.hashMap.get("md5"), new HashValueImpl(md5)) >= 0) || //$NON-NLS-1$
			   (sha1 != null && sha1.length() == 40 && Arrays.binarySearch(LedHashes.hashMap.get("sha-1"), new HashValueImpl(sha1)) >= 0)){ //$NON-NLS-1$
		...
		printTd(html, searcher, path, name, index, size, time, getInheritedShared(), virtualSize, virtualBase, sha1, tiger, md5,
				ed2k, bth, verify, uri, metadataAuto, metadataTime, metadataModified, rating, comments, shareTags,
				hitsTotal, uploadsTotal, cachedPreview, bogus);
