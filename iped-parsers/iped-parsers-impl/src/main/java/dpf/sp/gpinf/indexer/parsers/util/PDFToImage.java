...
/**
 * Classe utilitária para converter páginas de arquivos PDF para arquivos
 * de imagem.
 * 
 * @author Nassif
 *
		...
		try {
			if(!externalConversion) {
				if(PDFLIB.equals(PDFBOX)){
					...
					//BufferedImage buffImage = (BufferedImage) iceDoc.getPageImage(page, GraphicsRenderingHints.SCREEN, Page.BOUNDARY_CROPBOX, rotation, scale);
				...
				String[] cmd = {"java", "-cp", classpath, "-Xmx" + externalConvMaxMem, this.getClass().getCanonicalName(),  //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
						input.getAbsolutePath(), output.getAbsolutePath(), String.valueOf(page), PDFLIB, String.valueOf(RESOLUTION)};
			...
			LOGGER.warn("{} error creating image of page {} of {}: {}", Thread.currentThread().getName(), page, input.getAbsolutePath(), e.toString()); //$NON-NLS-1$
		}
		return success;
