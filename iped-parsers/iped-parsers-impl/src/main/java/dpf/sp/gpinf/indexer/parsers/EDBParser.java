...
public class EDBParser extends AbstractParser{
	...
	private File processTable(File tabFile, TemporaryResources tmp, EmbeddedDocumentExtractor extractor, ContentHandler handler) throws IOException, SAXException{
		Charset charset = Charset.forName("UTF-8"); //$NON-NLS-1$
		File html = tmp.createTemporaryFile();
		try(
			BufferedReader reader = new BufferedReader(new FileReader(tabFile));
			BufferedWriter writer = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(html), charset));
		){
			...
			writer.println("table {border-collapse: collapse; table-layout:fixed;} table, td, th {border: 1px solid black; word-wrap:break-word;}");
			writer.println("</style>");
			*/
			...
			while((line = reader.readLine()) != null){
				...
				for(int i = 0; i < values.length; i++){
					...
					if(value.length() <= 128 || !isHexBinary(value))
						...
						String name = TABLE_PREFIX + "_" + tabFile.getName() + "_col-" + colnames.get(i) + "_row" + row + ".data"; //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
					...
					if(header) writer.write("</b>"); //$NON-NLS-1$
					writer.write("</td>"); //$NON-NLS-1$
				}
				writer.write("</tr>\n"); //$NON-NLS-1$
				header = false;
