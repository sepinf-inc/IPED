	...
	public byte[] convertToHTML(List<DiscordRoot> drl, XHTMLContentHandler xhtml) throws IOException {
		...
		for (DiscordRoot dr : drl) {
			...
			if (!dr.getAuthor().getId().equals(userID)) {
				...
				out.println("			<TR>");
				out.println("				<TD>");
				out.println("					<img src='https://cdn.discordapp.com/avatars/" + dr.getAuthor().id + "/" + dr.getAuthor().avatar + ".png' alt='' width='50' height='50'>");
				out.println("				</TD>");
				out.println("				<TD>");
				out.println("					<span title='Channel ID=" + dr.getChannel_id() + ", UserID=" + dr.getAuthor().id + "'>" + dr.getAuthor().getName() + "<b>" + dr.getAuthor().getFullUsername() + "</b></span>");
				out.println("				</TD>");
				out.println("				<TD>");
			...
			if (dr.messageContent != null && !dr.messageContent.equals("")) {
				...
				out.println(		"<P>" + (dr.getEditedTimestamp() == null ? "" : "(Edit time:" + dateFormat.format(dr.getEditedTimestamp()) + ")") + "</P>"); 
				out.println("	</TD>");
				...
				if (dr.getAttachments() != null && dr.getAttachments().size() > 0) {
					...
					for (DiscordAttachment attachments : dr.getAttachments()) {
						out.println("<TR>");
						out.println("	<TD>");
						out.println("		<img src='" + attachments.getUrl() + "' alt='" + attachments.getFilename()	+ "' width='400px' height=''>");
						out.println("		<BR/>");
						out.println("		<a href='" + attachments.getUrl() + "'>" + attachments.getFilename() + "</a>");						
						out.println("	</TD>");
						out.println("</TR>");
					}
					out.println("</TABLE>");
