...
public class RARParser extends AbstractParser {
	...
	private static final Set<MediaType> SUPPORTED_TYPES = Collections.singleton(MediaType.application("x-rar-compressed")); //$NON-NLS-1$
		...
		try {
			...
			while (header != null && !Thread.currentThread().isInterrupted()) {
				if(header.isEncrypted())
					...
					throw new EncryptedDocumentException();
				/*
				 * Adiciona diretórios a um treemap para serem percorridos em profundidade
				 * pois são retornados na ordem contrária pelo junrar
				 */
	...
	private void parseSubitem(Archive rar, FileHeader header, String parent, ContentHandler handler, EmbeddedDocumentExtractor extractor) throws RarException, IOException, SAXException{
