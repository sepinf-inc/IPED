...
public class RawISOConverter {
	...
	private static int getISODataOffset(File file) {
	    ...
	    //CDROM sector 2048
	      if (matchMagic(magicISO9660, header, 32769) || 
	    		  matchMagic(magicISO9660, header, 34817) || 
	    		  matchMagic(magicUDF, header, 32769) ||
	    		  matchMagic(magicUDF, header, 34817))
	    	  return 0;
	      
	    //CDROM RAW sector 2352
	      if (matchMagic(magicISO9660, header, 37649) || 
		          matchMagic(magicISO9660, header, 40001) ||
		          matchMagic(magicUDF, header, 37649) ||
		          matchMagic(magicUDF, header, 40001))
	    	  return 16;
	      
	    //CDROM RAW XA sector 2352
	      if (matchMagic(magicISO9660, header, 37657) ||
		          matchMagic(magicISO9660, header, 40009) ||
		          matchMagic(magicUDF, header, 37657) ||
		          matchMagic(magicUDF, header, 40009))
	    	  return 24;
