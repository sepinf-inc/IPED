...
/**
 * Copied and adapted from org.apache.tika.parser.html.HtmlHandler.
 * This class had to be copied because it is package protected and
 * also required some adjusts
 * 
 * @author Fabio Melo Pfeifer
 *
 */
public class HtmlToHtmlHandler extends TextContentHandler {
    ...
    public void startElement(
        ...
        title.setLength(0);
        // Note: Code removed from original class. Do not handle DataURI
    }
        ...
        for (int att = 0; att < newAttributes.getLength(); att++) {
            ...
            if (normAttrName == null) {
                ...
                if (URI_ATTRIBUTES.contains(normAttrName)) {
                    // Note: Code removed. Do not remove src="data:" attrs 
    ...
    public void endElement(
        ...
        if ("SCRIPT".equals(name)) {
            ...
            if (scriptLevel == 0) {
                ...
                // Note: remove call to writeScripts (method removed)
            }
        }
    ...
    // Note: Removed method handleDataURIScheme, as we don't want to extract embedded data
    // Note: Removed method writeScript, as it's not needed here
