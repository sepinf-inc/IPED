...
public class LibpffPSTParser extends AbstractParser{
	...
	private static String[] types = {"Message", "Attachment", "Contact", "Meeting", "Task", "Appointment", "Activity", "Note", "Feed", "Document"}; //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$ //$NON-NLS-5$ //$NON-NLS-6$ //$NON-NLS-7$ //$NON-NLS-8$ //$NON-NLS-9$ //$NON-NLS-10$
			...
			if(!tested)
				try{
					...
					if(version.compareToIgnoreCase("20130722") < 0) //$NON-NLS-1$
						LOGGER.error(" Old version " + version + " of pffexport (libpff) detected: OST 2013 files will NOT be parsed!"); //$NON-NLS-1$ //$NON-NLS-2$
	...
	public void safeparse(InputStream stream, ContentHandler handler, Metadata metadata, ParseContext context) throws IOException, SAXException, TikaException {
		...
		try{
			...
			String[] cmd = {TOOL_PATH + TOOL_NAME, "-m", extractionMode, "-f", "html", "-t", tmpFile.getAbsolutePath(), file.getAbsolutePath()}; //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
		...
		for(File file : dir.listFiles()){
			...
			try{
				...
				for(int i = 0; i < types.length; i++){
					if(file.isDirectory() && fileName.startsWith(types[i]) && fileName.length() == types[i].length() + 5){
						try{
	...
	private int processFolder(String folderName, int parent) throws SAXException, IOException{
		...
		if(deleted) metadata.set(ExtraProperties.DELETED, "true"); //$NON-NLS-1$
	...
	private int handleMessage(File file, int parent) throws IOException, SAXException{
        ...
        if(!attachNames.isEmpty()) {
            preview.append("<b>" + Messages.getString("LibpffPSTParser.Attachments") + " (" + attachNames.size() + "):</b><br>"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
            for(String attach : attachNames) {
                preview.append(SimpleHTMLEncoder.htmlEncode(attach) + "<br>"); //$NON-NLS-1$
            }
            metadata.set(OutlookPSTParser.HAS_ATTACHS, "true"); //$NON-NLS-1$
        }
	...
	private void writeBody(File file, Metadata metadata, StringBuilder preview) throws IOException{
		...
		if(body.exists()){
			...
			Charset charset;
			try {
				AutoDetectReader adr = new AutoDetectReader(new ByteArrayInputStream(content));
				charset = adr.getCharset();
				adr.close();
				...
				if(l.length > 1){
					...
					if(!value.isEmpty()){
						...
						if(l[0].trim().equals("Client submit time")) //$NON-NLS-1$
							try {
								metadata.set(ExtraProperties.MESSAGE_DATE, df.parse(value.substring(0, value.length() - 14)));
	...
	private void handleAttachment(File file, int parent) throws IOException, SAXException{
		...
		try(InputStream is = Files.newInputStream(file.toPath())){
			...
			metadata.set(OutlookPSTParser.PST_ATTACH, "true"); //$NON-NLS-1$
			if(deleted) metadata.set(ExtraProperties.DELETED, "true"); //$NON-NLS-1$
