...
public class MRUKeyParser extends HtmlKeyParser {
                    ...
                    for (int j = 0; j < kvs.length; j++) {
                        ...
                        if (keyValue.getValueName().equals("MRUListEx")) {
                            ...
                            ByteArrayInputStream keyValueStream = new ByteArrayInputStream(
                                    generateMRUHtml(keyValue, 0, kmeta));
    ...
    private byte[] generateMRUHtml(KeyValue keyValue, int startOffset, Metadata kmeta)
        ...
        try {
            ...
            for (Iterator iterator = shellBagIndexes.iterator(); iterator.hasNext();) {
                ...
                if ((data[i] == 0x31) || (data[i] == 0x35) || (data[i] == 0x32) || (data[i] == 0x36)
                        || (data[i] == 0xb1)) {
                    int len = 0;
                    ...
                    fullPath.append(
                            new String(ArrayUtils.subarray(data, endNamePos + 2 - len, endNamePos + 4), "UTF-16LE"));
                    if (data[i] != 0x32) {
                        fullPath.append("/");
                    }
                    if (data[i] != 0x32) {
                        int j = i + 2;
                        filesize = (data[j] & 0xFF) + (data[j + 1] & 0xFF) * 256 + (data[j + 2] & 0xFF) * 256 * 256
                                + (data[j + 3] & 0xFF) * 256 * 256 * 256;
                        j += 4;
                        try {
    ...
    static Date dosDate2Date(byte[] data, int index) {
        ...
        return java.util.Date
                .from(LocalDateTime.of(year, month, day, hour, minute, second).atZone(ZoneId.of("UTC")).toInstant());
    }
}
