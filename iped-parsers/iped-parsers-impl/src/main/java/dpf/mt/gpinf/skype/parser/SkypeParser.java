...
public class SkypeParser extends AbstractParser {
        ...
        try {
            ...
            if (extractor.shouldParseEmbedded(metadata)) {
                ...
                for (SkypeConversation conv : convs) {
                    ...
                    if (extractor.shouldParseEmbedded(chatMetadata)) {
                        ByteArrayInputStream chatStream = new ByteArrayInputStream(r.generateSkypeConversationHtml(conv));
                        extractor.parseEmbedded(chatStream, handler, chatMetadata, false);
                    }
                    ...
                    if(extractMessageItems)
	                	for (SkypeMessage sm : conv.getMessages()) {
	                        ...
	                        if (extractor.shouldParseEmbedded(chatMetadata)) {
	                            ByteArrayInputStream chatStream = new ByteArrayInputStream(r.generateSkypeMessageHtml(sm));
	                            extractor.parseEmbedded(chatStream, handler, chatMetadata, false);
	                        }
	                    }
                }
                ...
                for (SkypeFileTransfer t : transfers) {
                    ...
                    tMetadata.set(TikaCoreProperties.TITLE, Messages.getString("SkypeParser.SkypeTransfer")+t.getFilename()); //$NON-NLS-1$
                    ...
                    if (extractor.shouldParseEmbedded(tMetadata)) {
                        ByteArrayInputStream chatStream = new ByteArrayInputStream(r.generateSkypeTransferenciaHtml(t, searcher));
                ...
                if (extractor.shouldParseEmbedded(meta)) {
                    ByteArrayInputStream chatStream = new ByteArrayInputStream(r.generateSkypeAccountHtml(a, contatos.size(), transfers.size(), msgCount ));
		...
		for(SkypeMessage sm : conv.getMessages()){
			if(sm.getAnexoUri() != null && sm.getAnexoUri().getCacheFile() != null)
			    ...
			    if(sm.getAnexoUri().getCacheFile().getLength() >= 512) {
			        metadata.add(ExtraProperties.LINKED_ITEMS, BasicProps.HASH + ":" + sm.getAnexoUri().getCacheFile().getHash());
			        if(sm.isFromMe())
	...
	private String getItemQuery(SkypeFileTransfer c, ItemSearcher searcher){
		...
		if(c.getFilePath() != null && !c.getFilePath().trim().isEmpty()){
			...
			if(searcher != null) path = searcher.escapeQuery(path);
