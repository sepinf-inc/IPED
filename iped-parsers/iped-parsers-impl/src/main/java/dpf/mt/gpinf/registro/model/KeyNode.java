...
public class KeyNode extends CellContent{
	...
	public Date getLastWrittenAsDate(){
        ...
        milis = milis - 11644473600000l; // diminui numero de milis segundos entre 01/01/1601 (filetime) e 01/01/1970 (javadate)
	...
	public FileTime getLastWrittenTimestamp(){
		...
		//long value2 = (buffer[0] & 0xFF) | (buffer[1] & 0xFF) << 8 | (buffer[2] & 0xFF) << 16 | (buffer[3] & 0xFF) << 24 | (buffer[4] & 0xFF) << 32 | (buffer[5] & 0xFF) << 40 | (buffer[6] & 0xFF) << 48 | (buffer[7] & 0xFF) << 56;
...
/*		int count = getValuesCount();
		...
		if(cellList.getCellContent() instanceof DataCell){
			...
			for (int i = 0; i < count; i++) {
				...
				buffer = null;
				HiveCell valueCell = reg.getCell(offset);
				result[pos] = (KeyValue) valueCell.getCellContent();
				pos++;
			}			
		}
