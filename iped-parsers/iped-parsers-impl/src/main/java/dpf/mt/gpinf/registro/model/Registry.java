...
public class Registry {
	...
	public static void main(String[] args) throws IOException {
		...
		RegistryFile rf = new RegistryFile(new File("/mnt/Ferramentas/JavaCode/BaseTeste/registro/baseteste/patrick.pdb/NTUSER.DAT"));
	...
	public static void montaArvore(KeyNode k, int level){
		String j="--";
		for(int i=0;i<level;i++){System.out.print(j);}
		System.out.println(k.getKeyName());
		
		KeyValue[] vs = k.getValues();
		if(vs!=null){
			for (int i = 0; i < vs.length; i++) {
				for(int vi=0;vi<level;vi++){System.out.print(j);}
				System.out.print("++");
