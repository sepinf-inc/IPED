	...
	public void parse(KeyNode kn, String title, boolean hasChildren, String keyPath, EmbeddedParent parent, ContentHandler handler,
		...
		if(kns!=null) {
			for (int i = 0; i < kns.size(); i++) {
				parseCertificateKey(kns.get(i), kns.get(i).getKeyName(), true, keyPath+"/"+kns.get(i).getKeyName(), parent, handler, metadata, context);
			}
		}
	}

	public void parseCertificateKey(KeyNode kn, String title, boolean hasChildren, String keyPath, EmbeddedParent parent, ContentHandler handler,
			Metadata metadata, ParseContext context) throws TikaException {
            			...
            			if(data[index]==0x30){
            				if(data[index+1]==(byte)0x82){
            					if(data[index+4]==0x30){
            						if (data[index+5]==(byte)0x82) break;
            					}
            				}
            			}
            		}
