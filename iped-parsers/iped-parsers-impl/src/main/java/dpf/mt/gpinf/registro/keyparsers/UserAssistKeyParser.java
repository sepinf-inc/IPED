...
public class UserAssistKeyParser extends HtmlKeyParser {
		        ...
		        if(kvs!=null) {
		        	for (int j = 0; j < kvs.length; j++) {
		    			...
		    			if(keyValue.getValueName().equals("MRUListEx")) {
		    	            ...
		    	            ByteArrayInputStream keyValueStream = new ByteArrayInputStream(generateUserAssistHtml(keyValue, 0, kmeta));
	...
	private byte[] generateUserAssistHtml(KeyValue keyValue, int startOffset, Metadata kmeta) throws UnsupportedEncodingException {
		try {
	        ...
	        long runCount = (data[4]& 0xFF)+(data[5]& 0xFF)*256 + (data[6]& 0xFF)*256*256 + (data[7]& 0xFF)*256*256*256;
