...
public class SQLiteCarver extends AbstractCarver {
	...
	public long getLengthFromHit(Item parentEvidence, Hit header) throws IOException {
		  ...
		  try{
			  ...
			  if((pageSize & (pageSize-1))!=0) return -1;// if not power of two


			  int vers = is.read();//write version
			  if((vers!=1) && (vers!=2)) return -1;
			  ...
			  return (long) numPages * (long) pageSize;
		  }catch(Exception e){
			  e.printStackTrace();
	...
	// rules retrieved from "https://www.sqlite.org/fileformat.html"
		  ...
		  try{
			  ...
			  if(formatWriteVersion[0]!=1 && formatWriteVersion[0]!=2) return false;
			  is.read(formatReadVersion);
			  if(formatReadVersion[0]!=1 && formatWriteVersion[0]!=2) return false;
			  is.read(unusedPageBytes);
			  if(unusedPageBytes[0]!=0) return false;
			  is.read(maxEmbeddedPayload);
			  if(maxEmbeddedPayload[0]!=64) return false;
			  is.read(minEmbeddedPayload);
			  if(minEmbeddedPayload[0]!=32) return false;
			  is.read(leafPayloadFraction);
			  if(leafPayloadFraction[0]!=32) return false;
			  is.skip(20);
			  is.read(schemaFormatNumber);
			  ByteBuffer wrapped = ByteBuffer.wrap(schemaFormatNumber);
			  int schemaFormatNumberInt = wrapped.getInt();
			  if(schemaFormatNumberInt!=1 && schemaFormatNumberInt!=2 && schemaFormatNumberInt!=3 && schemaFormatNumberInt!=4) return false;
			  is.skip(8);
			  is.read(dbTextEncoding);
			  wrapped = ByteBuffer.wrap(dbTextEncoding);
			  int dbTextEncodingInt = wrapped.getInt();
			  if(dbTextEncodingInt!=1 && dbTextEncodingInt!=2 && dbTextEncodingInt!=3) return false;
			  is.skip(12);
			  is.read(reservedZero);
			  for (int i = 0; i < reservedZero.length; i++) {
				if(reservedZero[i]!=0) return false;
			  }		  
