...
public class EMLCarver extends DefaultCarver {
	...
	public void notifyHit(Item parentEvidence, Hit hit) throws IOException {
		...
		if(hit.getSignature().isFooter()){
		    if(lastFooter == null || hit.getOffset() - lastFooter.getOffset() < hit.getSignature().getCarverType().getMaxLength())
		        lastFooter = hit;			
		}
	...
	private void carveFromLastFooter(Item parentEvidence) throws IOException {
        ...
        while((head = headersWaitingFooters.peekLast()) != null && lastFooter.getOffset() - head.getOffset() <= head.getSignature().getCarverType().getMaxLength()) {
	...
	public long getLengthFromHit(Item parentEvidence, Hit header) throws IOException {
	    System.out.println("EMLCarver getLengthFromHit");
		long distanceFromEnd = parentEvidence.getLength() - firstHeader.getOffset();
		long len = 0;
		if(distanceFromEnd > header.getSignature().getCarverType().getMaxLength()){
			len = header.getSignature().getCarverType().getMaxLength();
		}else{
			len = distanceFromEnd;
		}
		return len;
