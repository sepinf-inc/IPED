...
public class LibreOfficeViewer extends Viewer {
  ...
  public boolean isSpreadSheet(String contentType) {
    return contentType.startsWith("application/vnd.ms-excel") || contentType.startsWith("application/x-tika-msworks-spreadsheet") //$NON-NLS-1$ //$NON-NLS-2$
        || contentType.startsWith("application/vnd.openxmlformats-officedocument.spreadsheetml") || contentType.startsWith("application/vnd.oasis.opendocument.spreadsheet"); //$NON-NLS-1$ //$NON-NLS-2$
  ...
  public void constructLOFrame() {
    try {
      if(!System.getProperty("os.name").startsWith("Windows") &&
         ...
         LOGGER.error("LibreOffice viewer may not work properly. Install libreoffice-gtk2 and set environment var SAL_USE_VCLPLUGIN='gtk'");
    ...
    new Thread() {
      ...
      public void run() {
        ...
        synchronized (startLOLock) {
          /**
           * Código de proteção/reinicialização para casos de documentos com carregamento demorado ou 
           * infinito, utilizado quando usuário desiste de esperar e clica em outro documento.
           */
