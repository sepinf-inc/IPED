...
public class HexViewPanel extends JPanel {
      ...
      public void paint(Graphics g) {
        ...
        if (file != null) {
            ...
            for (long i = r1; i * cols < len; i++) {
              ...
              for (int j = 0; j < cols; j++) {
                ...
                if (b >= 0) {
                  ...
                  g2.drawString(String.format("%02x", b), x + width * 3 * j, y); //$NON-NLS-1$
                  g2.drawString((b < 32 || b > 127) ? "." : String.valueOf((char) b), x2 + width * j, y); //$NON-NLS-1$
                }
              }
              if (y > rc.height) {
                break;
          ...
          if (err) {
            ...
            g2.drawString(Messages.getString("HexViewPanel.ReadError"), 0, rc.y + rc.height / 2 - height); //$NON-NLS-1$
      ...
      public void keyPressed(KeyEvent e) {
        if ((e.getKeyCode() == KeyEvent.VK_C) && ((e.getModifiers() & KeyEvent.CTRL_MASK) != 0)) {
          if (selStart >= 0) {
            ...
            if (selMode == 1) {
              for (long i = a; i <= b; i++) {
                sb.append(String.format("%02x", getByte(i))); //$NON-NLS-1$
              }
            } else {
              for (long i = a; i <= b; i++) {
                int c = getByte(i);
  ...
  public void setFile(SeekableInputStream file) throws IOException {
    ...
    if (file != null) {
      ...
      offsetFormat = "%0" + offsetCols + "x"; //$NON-NLS-1$ //$NON-NLS-2$
    }
    err = false;
    memoBytes.clear();
    adjustSize();
  ...
  public void setFont(Font font) {
    ...
    width = fontMetrics.stringWidth("0"); //$NON-NLS-1$
